...
// Send print command to thermal printer via API
export const printOrderViaThermalPrinter = async (
  ...
  try {
    ...
    if (!result.success) {
      console.warn('[Print] Error:', result.message);
      return false;
    }
...
export const testPrinterConnection = async () => {
  try {
    const response = await fetch('/api/print/test-print', {
      method: 'POST'
    });
    ...
    return {
      success: false,
      message: 'Không thể kết nối máy in. Kiểm tra API server.'
    };
  }
};
...
export const updatePrinterConfig = async (printers: any[], defaultPrinter: string) => {
  try {
    const response = await fetch('/api/print/printers', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ printers, defaultPrinter })
    });

    const result = await response.json();
    return result.success;
  } catch (error: any) {
    console.error('[Print] Update config error:', error);
    return false;
  }
};
